Rcpp::sourceCpp("test-MSpline.cpp")
...
foo <- function(...) {
    mat <- mSpline(..., intercept = TRUE)
    imat <- mSpline(..., intercept = TRUE, integral = TRUE)
    d1mat <- deriv(mat)
    d2mat <- deriv(d1mat)
    d3mat <- deriv(d2mat)
...
res00 <- rcpp_mspline00(x, inter_knots, degree, bound_knots)
expect_equivalent(res, res00)

res01 <- rcpp_mspline01(x, inter_knots, degree, bound_knots)
expect_equivalent(res, res01)

res02 <- rcpp_mspline02(x, inter_knots, degree, bound_knots)
expect_equivalent(res, res02)

res03 <- rcpp_mspline03(x, inter_knots, degree, bound_knots)
expect_equivalent(res, res03)

res04 <- rcpp_mspline04(x, inter_knots, degree, bound_knots)
expect_equivalent(res, res04)

res05 <- rcpp_mspline05(x, inter_knots, degree, bound_knots)
expect_equivalent(res, res05)

## non-default constructor 1
res1 <- rcpp_mspline1(x, inter_knots, degree, bound_knots)
expect_equivalent(res, res1)

## non-default constructor 2
res2 <- rcpp_mspline2(x, 10, degree, bound_knots)
...
res31 <- rcpp_mspline3(x, degree, knot_seq)
...
res32 <- rcpp_mspline3(x, degree, knot_seq)
expect_equivalent(
...
res4 <- rcpp_mspline4(x, inter_knots, degree, bound_knots)
expect_equivalent(res, res4)

## conversion from BernsteinPoly
res5 <- rcpp_mspline5(x, inter_knots, degree, bound_knots)
expect_equivalent(res, res5)

## conversion from PeriodicMSpline
res6 <- rcpp_mspline6(x, inter_knots, degree, bound_knots)
