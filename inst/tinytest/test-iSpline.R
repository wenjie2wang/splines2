...
expect_error(iSpline(x, degree = c(2, NA)))
expect_error(iSpline(x, degree = NA))

## error if df has NA or negative
expect_error(iSpline(x, df = - 1))
expect_error(iSpline(x, df = c(2, NA)))
expect_error(iSpline(x, df = NA))

## error if knots has NA
expect_error(iSpline(x, knots = c(0.1, 0.5, NA)), "knots")
expect_error(iSpline(x, Boundary.knots = c(0.1, 0.5, NA)), "knots")

## error if boundary knots are inappropriate
expect_error(iSpline(x, Boundary.knots = 0.1), "knots")
expect_error(iSpline(x, Boundary.knots = c(0.1, 0.1)), "knots")
expect_error(iSpline(x, Boundary.knots = c(0.1, 0.5, 1)), "knots")
