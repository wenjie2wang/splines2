...
expect_eqt(dbs(x, derivs = 1, intercept = TRUE),
           splines::splineDesign(aKnots, x = x, derivs = 1))
expect_eqt(dbs(x, derivs = 2, intercept = TRUE),
...
expect_eqt(dbs(x, derivs = 1, knots = knots, intercept = TRUE),
           splines::splineDesign(aKnots, x = x, derivs = 1))
expect_eqt(dbs(x, derivs = 2, knots = knots, intercept = TRUE),
           splines::splineDesign(aKnots, x = x, derivs = 2))
expect_eqt(
...
expect_eqt(dbs(x, 1, knots = knots, degree = 4, intercept = TRUE),
           splines::splineDesign(aKnots, x, ord, derivs = 1))
expect_eqt(dbs(x, 2, knots = knots, degree = 4, intercept = TRUE),
           splines::splineDesign(aKnots, x, ord, derivs = 2))
expect_eqt(dbs(x, 3, knots = knots, degree = 4, intercept = TRUE),
...
expect_eqt(dbs(x), v2$dbs(x))

## cubic splines with specified df
expect_eqt(dbs(x, df = 5),
           v2$dbs(x, df = 5))

## cubic splines with specified internal knots
expect_eqt(dbs(x, knots = knots),
           v2$dbs(x, knots = knots))

## qudractic splines without internal knots
expect_eqt(dbs(x, degree = 2L),
           v2$dbs(x, degree = 2L))

## complete basis with intercept
expect_eqt(dbs(x, intercept = TRUE),
           v2$dbs(x, intercept = TRUE))

## specified knots
expect_eqt(dbs(x, knots = knots, intercept = TRUE),
           v2$dbs(x, knots = knots, intercept = TRUE))

## specified df
expect_eqt(dbs(x, df = 6, intercept = TRUE),
           v2$dbs(x, df = 6, intercept = TRUE))

## degree zero
expect_eqt(dbs(x, df = 5, degree = 0),
           v2$dbs(x, df = 5, degree = 0))
expect_eqt(dbs(x, df = 5, degree = 0, intercept = TRUE),
