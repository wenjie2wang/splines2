...
// [[Rcpp::export]]
Rcpp::NumericMatrix rcpp_iSpline_basis(
    const arma::vec& x,
    ...
    )
{
    ...
    Rcpp::NumericMatrix out {
        splines2::arma2rmat(is_obj.basis(complete_basis))
            };
    // add attributes
    ...
    out.attr("degree") = is_obj.get_degree();
...
// [[Rcpp::export]]
Rcpp::NumericMatrix rcpp_iSpline_derivative(
    const arma::vec& x,
    ...
    )
{
    ...
    Rcpp::NumericMatrix out {
        splines2::arma2rmat(is_obj.derivative(derivs, complete_basis))
            };
    // add attributes
    out.attr("dimnames") = Rcpp::List::create(
        R_NilValue, splines2::char_seq_len(out.ncol())
...
// [[Rcpp::export]]
Rcpp::NumericMatrix rcpp_iSpline_integral(
    const arma::vec& x,
    const unsigned int df,
    const unsigned int degree,
    const arma::vec& internal_knots,
    ...
    )
{
    ...
    Rcpp::NumericMatrix out {
        splines2::arma2rmat(is_obj.integral(complete_basis))
            };
    // add attributes
    out.attr("dimnames") = Rcpp::List::create(
        R_NilValue, splines2::char_seq_len(out.ncol())
