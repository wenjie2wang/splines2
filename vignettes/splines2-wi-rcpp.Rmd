...
the *complete spline basis functions* (different with `df` in the R interface)
is specified.
...
(different with `df` in the R interface) is specified.
...
The `PeriodicMSpline` class is intended for constructing the periodic M-splines,
which provides the same set of non-default constructors with `BSpline`.
The only difference is that a *simple knot sequence* must be specified for the
third non-default constructor that takes a knot sequence.
