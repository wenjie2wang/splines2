...
The **splines2** package is designed to be a supplementary package on splines.
It provides functions constructing a variety of regression spline bases that are
not available from the **splines** package shipped with base R.  Most functions
have a very similar user interface with the function `splines::bs`.
**splines2** `r packageVersion("splines2")` provides function constructing
B-splines, integral of B-splines, M-splines and its integral (I-splines), convex
splines (C-splines), and their derivatives.  Compared with the **splines**
package, **splines2** allows piecewise constant basis for B-splines.  Also, it
provides a more user-friendly function interface, more consistent handling on
`NA`'s for spline derivatives.  Most of the implementation has been re-written
in C++ with help of Rcpp and RcppArmadillo since v0.3.0.


In this vignette, we introduce the basic usage of most functions in the package
through examples. The details of function syntax are available in the package
manual and thus will be not discussed.
...
```{r msMat}
stopifnot(all.equal(msOut, deriv(isOut), check.attributes = FALSE))
```
...
is scaled to have unit height at the right boundary knot.
